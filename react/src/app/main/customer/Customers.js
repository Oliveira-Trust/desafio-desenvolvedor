...
import { destroyAll, readAll } from "./Customer.service";
import CustomerModalForm from "./CustomerModalForm";
import CustomerModalShow from "./CustomerModalShow";
...
class Customers extends React.Component {
    ...
    handleChangeFilter = (e) => {
        ...
        dataFiltered = dataFiltered.concat(this.state.dataFull).filter(d => { return d.name.toLowerCase().includes(e.target.value.toLowerCase()) })
        dataFiltered = dataFiltered.concat(this.state.dataFull.filter(d => { return d.email.toLowerCase().includes(e.target.value.toLowerCase()) }))
        dataFiltered = dataFiltered.concat(this.state.dataFull.filter(d => { return d.cpf.toLowerCase().includes(e.target.value.toLowerCase()) }))
        dataFiltered = dataFiltered.concat(this.state.dataFull.filter(d => { return d.phone.toLowerCase().includes(e.target.value.toLowerCase()) }))
        dataFiltered = dataFiltered.concat(this.state.dataFull.filter(d => { return d.address.toLowerCase().includes(e.target.value.toLowerCase()) }))

        this.setState({ data: [...(new Set(dataFiltered))] })
    }
    ...
    render() {
        return (
            <>
                ...
                <Container fluid>
                    <Row>
                        <Col><h3>Lista de Clientes</h3></Col>
                        <Col md="auto">
                ...
                <Table striped bordered hover>
                    <thead>
                        <tr>
                            ...
                            <th onClick={() => this.handleClickOrder('email')}>E-Mail</th>
                            <th onClick={() => this.handleClickOrder('cpf')}>CPF</th>
                            <th onClick={() => this.handleClickOrder('phone')}>Telefone</th>
                            <th onClick={() => this.handleClickOrder('address')}>Endereço</th>
