  ...
  >
    <template v-slot:item="{ item }">
      <tr>
        ...
        <td>
          <v-icon :color="item.email_sent_at ? 'green' : 'red'">
            {{ item.email_sent_at ? 'mdi-email-check' : 'mdi-email' }}
          </v-icon>
        </td>
        <td>
    ...
    <template v-slot:no-data>
      <v-alert :value="true" color="error" icon="mdi-alert">
        {{ falha ? 'Erro ao carregar histórico' : 'Nenhum dado disponível' }}
      </v-alert>
    </template>
  ...
  <v-dialog v-model="dialog" max-width="600">
    <v-card>
      ...
      <v-card-text v-if="selectedItem">
        ...
        <div>Taxa Cambio: R$ {{ formatCurrency(selectedItem.tax_total) }}</div>
        <br>
...
export default {
  ...
  data() {
    return {
      ...
      perPage: 10, // Itens por página padrão
  ...
  methods: {
    async fetchHis({page, itemsPerPage, sortBy }) {
      try {
        this.loading = true;
        const response = await QuoteService.getQuoteHistory(this.user, page, itemsPerPage, sortBy);
