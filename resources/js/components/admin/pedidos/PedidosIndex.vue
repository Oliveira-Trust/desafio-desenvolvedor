...
<div>
	<div class="row">
		...
		<div class="col-xl-2 col-sm-6 py-4">
			<a href="/admin/pedidos/create" class="btn btn-success btn-sm float-right"><i class="fas fa-plus"></i> Adicionar</a>
		</div>
	</div>
	<div class="row">
				...
				<table class="table table-striped table-bordered table-hover table-sm">
					...
					<tbody v-if="items.length > 0">
						<tr v-for="(item, i) in items" :key="i" :class="setClass(item.oid) == true ? '': 'table-warning'">
							...
							<td class="align-middle">{{getStatus(item.status) }}</td>
							<td class="align-middle">{{formatDate(item.paid_at, 'DD/MM/YYYY')}}</td>
	...
	export default {
		...
		methods: {
			...
			async getResults() {
				...
				const response = await axios.get('/admin/buscar-pedidos', {params: {page, term, field, sortBy, sortDirection, info}});
			...
			remove(item) {
				this.$swal.fire({
					...
					if(result.value) {
						axios.delete(`/admin/pedidos/${item.oid}`).then((response) => {
							this.$swal.fire('', response.data.message, 'success')
							this.items = this.items.filter((it) => it.oid !== item.oid);
							this.getResults();
						}).catch((e) => {
