...
@section('main')
        ...
        <div class="col-sm-12 text-right">
            <button style="margin-bottom: 10px" id="fin-compra" class="btn btn-success "
                    data-url="{{route('purchase.store')}}">Finalizar compra
            </button>
        </div>

        <table class="table table-bordered" id="create-purchase">
            <thead>
            <tr>
            ...
            @if($products->count())
                ...
                @foreach($products as $key => $product)
                    <tr id="tr_{{$product->id}}" class="lines">
                        <td width="50px"><input type="checkbox" name="check" class="sub_chk" data-id="{{$product->id}}"></td>
                        <td>{{ $product->name }}</td>
                            ...
                            <div class="form-group row">
                                <div class="col-md-12">
                                    <input  type="number" max="{{$product->inventory->amount}}" min="0" class="form-control @error('amount') is-invalid @enderror amount-{{$product->id}}"  name="amount"
                                           value="{{ old('amount')? old('amount'): 0 }}" required autocomplete="amount">
                                </div>
                            </div>
                        </td>
                    </tr>
...
@endsection
@section('js')
    <script src="{{ asset('js/purchase-create.js') }}" defer></script>
@endsection
