  ...
  <main>
    ...
    <div class="row g-12">
      ...
      <div class="col-md-12 col-lg-12">
        ...
        <h4 class="mb-3">Editar Itens do Pedido </h4>
...
<form action="{{ route('update_pedido')}}" method="POST">
       @csrf
   ...
   <div class="col-3">
              <div class="input-group has-validation">
                <span class="input-group-text">Status Pedido</span>
            ...
            <table id="table" data-toggle="table"  data-search="false" data-pagination="false" data-click-to-select="true" 
               ...
               <thead>
                 ...
                 @foreach($produto as $class)
                    ...
                    <td> <input type="text" class="form-control" id="qtd_item[{{$class->id}}]" name="qtd_item[{{$class->id}}]" onblur="JS:total_item('{{$class->id}}','{{$class->valor_produto}}')" value="{{Get_info_item_pedido($pedido->id,$class->id,'Q')}}"> </td>
                    <td> <input type="text" class="form-control" id="desconto_item[{{$class->id}}]" name="desconto_item[{{$class->id}}]" onblur="JS:total_item('{{$class->id}}','{{$class->valor_produto}}')" value="{{Get_info_item_pedido($pedido->id,$class->id,'D')}}"> </td>
                    <td>
                    <input type="text" id="total_item[{{$class->id}}]" class="form-control"   name="total_item[{{$class->id}}]" value="{{Get_info_item_pedido($pedido->id,$class->id,'L')}}" readonly>
  ...
  var $table = $('#table')
    ...
    jQuery.each( total_pedido_var, function( i, field ) {
             if(field.name.substring(0,7) == "id_item" )
                                                {
                                                 var valor_item = document.getElementById("valor_item["+field.value+"]").value;
                                                total_item(field.value,valor_item);
                                                }
    	});
