            ...
            @foreach($historic as $item)
                <div class="w-full p-2 rounded-lg border-base-red border border-l-8 text-sm shadow">
                    <div class="flex flex-row justify-between">
                        ...
                        <p><span class="text-base-red font-[600]">Conversão com taxas: </span>{{$item->amount_after_taxes}}</p>
                    </div>

                    <div class="flex flex-row justify-between">
                        <p>{{$item->source_currency}} <span class="text-base-red font-[600]">></span> {{$item->destination_currency}}</p>
                    ...
                    <div class="flex flex-row justify-between mt-2">
                        <p><span class="text-base-red font-[600]">Valor da moeda:</span> {{number_format((1 / $item->rate), 2, ',', '.')}}</p>
