<div>
    ...
    Valor utilizado para conversão descontando as taxas: {{ $conversion['target_amount'] - $conversion['amount_fee'] - $conversion['payment_fee'] }} <br>
    <hr>
    <b>Valor comprado em "Moeda de destino":</b> {{ $conversion['target_amount'] }}
    <hr>
    <small>Cotação realizada em {{ $conversion['created_at']->format('d/m/Y H:i:s') }}</small>
</div>
